<?php
  ...
  $options  = array( PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION);
	...
	$sql = "SELECT * from
			...
			where Inventario.idAlmacen = $sucursal and Linea.idLinea = $linea
	...
	$sql2 = "SELECT nombre from Linea where idLinea =$linea;";
    $query2 = $connection->query($sql2);
    $row2 = $query2->fetch(PDO::FETCH_ASSOC);
    $lin = $row2["nombre"];
    $sql3 = "SELECT name from Almacen where idAlmacen =$sucursal;";
	...
	class PDF Extends FPDF
	{
		...
		$pdf->Cell(200,5, 'Inventario General por Linea', 0, 1, 'C');
		$pdf->SetFont('Arial', '', 12);
		$pdf->Cell(200,5, 'Fecha de venta: '.$fecha ,0,1,'C');
		$pdf->Cell(200,5, 'Sucursal: '.$suc,0,1,'C');
		$pdf->Cell(200,5, 'Linea: '.$lin,0,0,'C');
		$pdf->Ln(10);
